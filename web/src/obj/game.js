    ...
    Game.prototype.draw = function (c) {
        ...
        if (screens.length) {
            ...
            for (var i = 0; i < screens.length; i++) {
                ...
                var percent = (Math.min(screen[0] + screen[2], box.x + box.width) - Math.max(screen[0], box.x)) * (Math.min(screen[1] + screen[3], box.y + box.height) - Math.max(screen[1], box.y)) / (box.width * box.height);
