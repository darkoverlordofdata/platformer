            ...
            for (var i = 0; i < this.boxes.length; i++) {
                ...
                if (b.y + b.height > box.y && box.y + box.height > b.y) {
                    ...
                    if (slide) {
                        ...
                        if (b.type == Type.RampBottomLeft && fromRight && aboveBottom)
                            box.y = Math.min(box.y, b.y + b.height * tLeft - box.height);
                        else if (b.type == Type.RampBottomRight && fromLeft && aboveBottom)
                            box.y = Math.min(box.y, b.y + b.height * (1 - tRight) - box.height);
                        else if (b.type == Type.RampTopLeft && fromRight && belowTop)
                            box.y = Math.max(box.y, b.y + b.height * (1 - tLeft));
                        else if (b.type == Type.RampTopRight && fromLeft && belowTop)
                            box.y = Math.max(box.y, b.y + b.height * tRight);
                        else
                            skip = false;
                    }
                    else {
                        ...
                        if (b.type == Type.RampBottomLeft && fromRight && aboveBottom)
                            dx = Math.max(dx, b.x + b.width * tBottom - box.x);
                        else if (b.type == Type.RampBottomRight && fromLeft && aboveBottom)
                            dx = Math.min(dx, b.x + b.width * (1 - tBottom) - box.x - box.width);
                        else if (b.type == Type.RampTopLeft && fromRight && belowTop)
                            dx = Math.max(dx, b.x + b.width * (1 - tTop) - box.x);
                        else if (b.type == Type.RampTopRight && fromLeft && belowTop)
                            dx = Math.min(dx, b.x + b.width * tTop - box.x - box.width);
                        else
                            skip = false;
                    }
                    if (skip)
                    ...
                    if (dx < 0 && box.x >= b.x + b.width && box.x + dx < b.x + b.width)
                        dx = b.x + b.width - box.x;
                    else if (dx > 0 && box.x + box.width <= b.x && box.x + box.width + dx > b.x)
                        dx = b.x - box.x - box.width;
                }
            }
            box.x += dx;
        }
        ...
        if (dy) {
            ...
            for (var i = 0; i < this.boxes.length; i++) {
                ...
                if (b.x + b.width > box.x && box.x + box.width > b.x) {
                    ...
                    if (b.type == Type.RampBottomLeft)
                        top = b.y + b.height * tLeft;
                    else if (b.type == Type.RampBottomRight)
                        top = b.y + b.height * (1 - tRight);
                    else if (b.type == Type.RampTopLeft)
                        bottom = b.y + b.height * (1 - tLeft);
                    else if (b.type == Type.RampTopRight)
                        bottom = b.y + b.height * tRight;
                    // Adjust vertical distance to avoid collisions
                    if (dy < 0 && box.y >= bottom && box.y + dy < bottom)
                        dy = bottom - box.y;
                    else if (dy > 0 && box.y + box.height <= top && box.y + box.height + dy > top)
                        dy = top - box.y - box.height;
                }
            }
            box.y += dy;
        }
